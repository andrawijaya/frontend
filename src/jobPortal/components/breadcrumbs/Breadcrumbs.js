...
export default function ListBreadcrumbs(props) {
    ...
    }, [props.menu, props.name, props.subMenu, props.subMenu2, props.subMenuType, props.submenu2, props.url])
                ...
                <div>
                    {subMenu === '' ?
                        <Breadcrumbs className={classes.breadcrumbs} separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb">
                            <Link style={{ color: 'black', fontSize: '14px', fontWeight: '500' }} to={url}>
                                {name}
                            </Link>
                        </Breadcrumbs>
                        :
                        subMenu2 !== '' ?
                            <Breadcrumbs className={classes.breadcrumbs} separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb">
                                ...
                                <Typography style={{ fontSize: '14px' }} color="textPrimary">{subMenuType}</Typography>
                            </Breadcrumbs>
                            :
                            <Breadcrumbs className={classes.breadcrumbs} separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb">
                                <Link style={{ color: 'black', fontSize: '14px', fontWeight: '500' }} to={url}>
                                ...
                                <Typography style={{ fontSize: '14px' }} color="textPrimary">{subMenu}</Typography>
                            </Breadcrumbs>
                    }
                </div>
                : null}
