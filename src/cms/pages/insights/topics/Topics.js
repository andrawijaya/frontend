...
export default function Topics() {
    ...
    const handleDeleteData = (id, name, count) => {
        if (toInteger(count) > 0) {
            setShowAlert(true)
            setAlerType(2)
            setAlertMessage("Data can't be deleted because asigned in another table!")
        } else {
            setIdDelete(id)
            setShowAlert(true)
            setAlerType(3)
    ...
    const confirmDeleteData = () => {
        topicsCMSAPIs.deleteTopicById(idDelete)
            .then((res) => {
                setisFetch(!isFetch)
                setShowAlert(true)
                setAlerType(1)
            ...
            <div className="wrap-inside-insights">
                <div className="heading-title">
                    ...
                    <div className="wrap-breadcrumb div_flex" style={{ paddingBottom: '10px', alignItems: 'flex-start' }}>
                        <ListBreadcrumbs menu={'list'} name={'Insights'} subMenu={`${'Topics'}`} url={'/website-cms/topics'} />
                        <div className="wrap-btn">
                            <Link to={{ pathname: "/website-cms/topics/form/add/1" }}>
                            ...
                            {!isEmpty(data) && data.data.map((v, i) => (
                                <div key={i} className="content-table row">
                                    ...
                                    <div className="col-3" style={{ justifyContent: 'space-evenly', display: 'flex', textAlign: 'center' }}>
                                        <Link to={{ pathname: `/website-cms/topics/form/edit/${v.id}` }} className="a-href-label">Edit</Link>
                                        <Link to={{ pathname: `/website-cms/topics/form/detail/${v.id}` }} className="a-href-label">Detail</Link>
                                        <label onClick={() => handleDeleteData(v.id, v.name, v.article_count)} className="a-href-label">Delete</label>
                                    </div>
                                </div>
                            ))}
                        </div>
